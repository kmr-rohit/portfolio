...
<header class="top">
	<a href="/" class="name" aria-current={$page.url.pathname === '/' ? 'page' : undefined}>
		{site.name}
	</a>

	<nav aria-label="Primary">
		{#each routes as route (route.link)}
			<a href={route.link} aria-current={isActive(route.link) ? 'page' : undefined}>
	...
	<div class="top-socials">
		...
		{/if}
		{#if email}
			<a href={email.href} class="icon-btn no-highlight" aria-label="Email" title="Email">
				<svg
					width="17"
					height="17"
					viewBox="0 0 24 24"
