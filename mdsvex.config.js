...
const prettyCodeOptions = {
  // theme: 'github-dark',
  theme: JSON.parse(
    readFileSync(resolve(__dirname, "./static/github-dark.json"), "utf-8")
  ),
    ...
    visit(tree, (node) => {
      if (node?.type === "element" && node?.tagName === "div") {
        ...
        if (
          ...
          "value" in titleElement.children[0]
        ) {
  ...
  return async (tree) => {
    ...
    visit(tree, (node) => {
      if (
        ...
        if (
          /(?:^|\n)\s*make\s+-C\s+(?:\.\/)?labs\/kubernetes(?:\s|$)/.test(
            toString(codeEl)
          )
        ) {
          node.properties["data-lab-command"] = true;
          node.properties[
            "data-lab-command-id"
          ] = `lab-command-${labCommandIndex}`;
          labCommandIndex += 1;
        }
